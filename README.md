...
低平地を詳しく見る場合は背景を「解析用色別標高図」にし、最小・最大標高を入力するか「低平地 0～10m」「表示範囲から推定」を使います。再配色は新しい256px DEMタイルの受信時とレンジ変更時だけ行い、地図移動の各フレームでは行いません。自動推定もボタンを押した時だけ16地点を標本抽出します。等高線は端末でDEMから生成せず、国土地理院の試験公開ベクトルを選択時だけ描画します。どちらも初期状態では無効で、連続的な再計算を避けています。古い端末で発熱・電池消費が気になる場合は、固定配色の「色別標高図」を使い、等高線をオフにしてください。
