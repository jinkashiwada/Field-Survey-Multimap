...
「表示範囲から推定」は、手動ボタンまたはペインごとの自動設定で、表示範囲の4×4＝16地点からDEM10Bを標本抽出する。自動設定でもmoveend後300msに一度だけ実行し、同じViewを共有する複数の自動ペインは一回の標本取得結果を共有する。同じレンジならReducerが更新を省略する。レンジと自動設定はhashへ保存する。等高線は端末でDEMから計算せず、試験公開MVTの既成線を既定オフで表示する。
...
河川は公式MVTのズーム別一般化を利用する。全国規模の過密表示を避けてZL7から表示し、ZL7では `ftCode=55301`（河川幅・大）と `55302`（中）、ZL8以上では詳細河川を描く。MVTに管理区分がないため、一級河川・直轄・都道府県管理の推定分類は行わない。
